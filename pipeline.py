...
from conf import OUTPUT_DIR, SAMPLE_FIRST, SET_COMMON_FEATURES_TO_ZERO, REMOVE_FEATURES_ALLZERO, VIGILANCE_RANGE, paths, params, mode_params, NGRAMS, SOUNDCLASSES, FEATURES_SET
...
def main():
    ...
    for param_name in params:
        ...
        if param["type"] == bool:
            parser.add_argument(
                f"--{param_name}", default=param["default"], action='store_true')
        else:
            parser.add_argument(
                f"--{param_name}", type=param["type"], default=param["default"])
    args = parser.parse_args()
    print(args)

    config_string = "train_test" if args.train_test else ""
    config_string += "".join([f"-{param_name}={param_value}" for param_name, param_value in vars(args).items(
    ) if param_name not in mode_params and param_name != "language" and (args.single_run or param_name != "vigilance_single_run")])
    ...
    forms_onehot, inflections, inflection_classes, _, ngram_inventory = data.create_language_dataset(df_language, language, Ngrams=NGRAMS,
                                                                                                     sample_first=SAMPLE_FIRST, features_set=FEATURES_SET, set_common_features_to_zero=SET_COMMON_FEATURES_TO_ZERO, remove_features_allzero=REMOVE_FEATURES_ALLZERO, soundclasses=SOUNDCLASSES)
    ...
    if args.single_run:
        art(forms_onehot, ngram_inventory, inflections, inflection_classes, language, config_string, n_runs=1, vigilances=[
            args.vigilance_single_run], shuffle_data=True, repeat_dataset=True, visualise_clusters=True, train_test=args.train_test, eval_intervals=args.eval_intervals)
    ...
    if args.baseline:
        ...
        records_baseline.append(random_baseline(
            inflections, n_inflection_classes=len(inflection_classes)))
